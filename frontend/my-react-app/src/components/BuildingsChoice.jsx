...
const BuildingsChoice = ({data}) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if( isNaN(parseInt(zipCode,10)) || 
    isNaN(parseInt(numberOfIndividualApartments,10)) ){
      ...
      {showAddNew && (
        <div ref={targetDivRef} className="login-form">
          ...
          <form onSubmit={handleSubmit} >
            ...
            <label>
              ZipCode:
              <input type="text" name="zipCodetxt" value={zipCode} onChange={(e) => setZipCode(e.target.value)} />
            </label>
            <br />
            <label>
              City:
            ...
            <input className="text" type="text" name="numberOfIndividualApartmentstxt" value={numberOfIndividualApartments} onChange={(e) => setNoia(e.target.value)} />
            </label>
            <br />
          ...
          <button className='close_login' onClick={toggleAddNew}>X</button>
        </div>
      )}
    </>
  )
